...
export const CardSmall = ({data}) => {
  ...
  return (
    ...
    {latestBlogs.map((blog) => (
      <Card key={blog.id} blog={blog} href={`blog/${blog.id}`} w={"w-[44%]"} flex={"flex"} showContent={false} imgh={"h-[100px] min-w-[170px] "}  padd={"pl-[15px]"} centerText={"flex flex-col justify-center"}/>
    ))}
  </div>
  )
}
