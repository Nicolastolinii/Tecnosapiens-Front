...
const Card = ({ blog, showContent = true, w, flex, mb, imgh, imgmargin, padd, centerText, href }) => {
  return (
    <div className={`w-full ${w} ${mb} ${flex} bg-white rounded-xl shadow-custom p-4`}>
      
      <Link to={href}>
        <img className={`bg-cover object-cover w-full rounded-lg ${imgh}  ${imgmargin}`} src={`data:image;base64,${blog.imagen}`} alt={blog.title} />
      </Link>
